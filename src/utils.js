...
exports.logDebug = ( message ) => {
	...
	const d = new Date(); 
	const parsedTime = `${ TerminalColors.Dim }[${d.toLocaleTimeString()}]${ TerminalColors.Reset }`;
	const callerFile = `${TerminalColors.FgYellow}[${_getCallerFile()}]${ TerminalColors.Reset }`
...
exports.normalizeGroupName = ( groupName ) => {
  	return groupName.toLocaleLowerCase().split(" ").map(e=>e.replace(/^[A-Za-z0-9_.]+$/g, "")).join("-")
}
