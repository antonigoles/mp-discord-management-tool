...
const registerHandler = async (client) => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.commandName === COMMAND_NAME) {
      ...
      if (
        !(await databaseManager.isGroupInDb(interaction.guild.id, groupName))
      ) {
        interaction.reply({ content: "😨 Taka grupa nie istnieje" });
        return;
      }

      const groupData = await databaseManager.getGroupByName(
      ...
      if (groupData.students.length <= 0) {
        interaction.reply({
          content:
            "**Wygląda na to że w tej grupie nie ma jeszcze uczniów...**\nDodaj uczniów komendą: `/addstudents`",
        });
        return;
      }
      ...
      const taskEmbed = new MessageEmbed()
        ...
        .addFields([
          ...groupedStudents.map((studentGroup) => {
            return {
              name: `\`Osoba${Utils.setLengthString(
                "",
                10 + Math.floor(3.3 * tasks.length)
              )}Progress\``,
              value: studentGroup
                .map((studentId) => {
                  ...
                  const nickname =
                    mem != undefined
                      ? mem.displayName
                      : "UCZEŃ WYSZEDŁ Z SERWERA";
                  const parsedName = `${Utils.setLengthString(nickname, 21)}`;
                  return `\`${parsedName}: \`${tasks
                    .map((_, i) => emojiMap[`red${i + 1}`])
                    .join(" ")}`;
                })
                .join("\n"),
            };
    ...
    originalMessage.embeds[0].setFields([
      ...groupedStudents.map((studentGroup) => {
        return {
          name: `\`Osoba${Utils.setLengthString(
            "",
            10 + Math.floor(3.3 * updatedTracker.taskCount)
          )}Progress\``,
          value: studentGroup
            .map((studentId) => {
              const mem = members.find((u) => u.id == studentId);
              const nickname =
                mem != undefined ? mem.displayName : "UCZEŃ WYSZEDŁ Z SERWERA";
              const parsedName = `${Utils.setLengthString(nickname, 21)}`;
              return `\`${parsedName}: \`${Object.keys(
                updatedTracker.progress[studentId]
              )
                .map((task) =>
                  updatedTracker.progress[studentId][task]
                    ? emojiMap[`green${Number(task) + 1}`]
                    : emojiMap[`red${Number(task) + 1}`]
                )
                .join(" ")}`;
            })
            .join("\n"),
...
exports.command = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName("task1").setDescription("task1").setRequired(true)
  )
  .addStringOption((option) =>
    option.setName("task2").setDescription("task2").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task3").setDescription("task3").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task4").setDescription("task4").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task5").setDescription("task5").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task6").setDescription("task6").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task7").setDescription("task7").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task8").setDescription("task8").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task9").setDescription("task9").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task10").setDescription("task10").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task11").setDescription("task11").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task12").setDescription("task12").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task13").setDescription("task13").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task14").setDescription("task14").setRequired(false)
  )
  .addStringOption((option) =>
    option.setName("task15").setDescription("task15").setRequired(false)
  );
